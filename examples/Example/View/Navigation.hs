...
module Example.View.Navigation where
...
exampleMenu current = do
  ...
  case current of
    Basics -> do
      mapM_ anchorLink [minBound .. maxBound :: Basics]
    -- exampleLink (Intro Pages) ~ sub
    -- exampleLink (Intro Views) ~ sub
    -- exampleLink (Intro HyperViews) ~ sub
    -- exampleLink (Intro ViewFunctions) ~ sub
    -- exampleLink (Intro (CSS CSSAll)) ~ sub
    _ -> none
  exampleLink CSS
 ...
 where
  ...
  anchorLink :: (Show a, PageAnchor a) => a -> View c ()
  anchorLink a = do
    tag "a" ~ sub . menuItem . cls "nav-active" @ att "href" ("#" <> pageAnchor a) $ do
      text $ navEntry a
