...
-- Forms can be pretty simple
data ContactForm = ContactForm
  { name :: Text
  , age :: Int
  }
  deriving (Generic, FormParse)
...
formView = do
  form Submit (gap 10 . pad 10) $ do
    ...
    -- You must make sure these names match the field names used by formData / FormParse
    field (FieldName "name") id $ do
      label "Contact Name"
      input Username (inp . placeholder "contact name")

    field (FieldName "age") id $ do
      label "Age"
