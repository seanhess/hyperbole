...
data UserForm f = UserForm
  ...
  deriving (Generic)
instance FormParse (UserForm Identity)
instance Fields UserForm Validated
...
formView v = do
  ...
  form Submit (gap 10 . pad 10) $ do
    ...
    -- what is its type? we don't know
    field f.user (valStyle f.user.value) $ do
      label "Username"
      input Username (inp . placeholder "username")

      case f.user.value of
        Invalid t -> el_ (text t)
        Valid -> el_ "Username is available"
        _ -> none

    field f.age (valStyle f.age.value) $ do
      label "Age"
      input Number (inp . placeholder "age" . value "0")
      el_ $ invalidText f.age.value

    field f.pass1 (valStyle f.pass1.value) $ do
      label "Password"
      input NewPassword (inp . placeholder "password")
      el_ $ invalidText f.pass1.value
