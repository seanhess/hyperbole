The second argument of `liveApp` is an `Effect` monad which returns a `Response`. We will rarely return a `Response` directly, so we use the `runPage` function to turn `Page es '[...]` into an `Eff [..] Response`
